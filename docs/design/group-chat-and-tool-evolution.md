...
**已实现**: `DebugEventBus.emit_pipeline()` 发射语义管线事件，通过现有 SSE 流推送。前端 `DebugEventList.vue` 识别 `direction=pipeline` 事件并按类型渲染卡片和详情：

- ~~工具调用决策: 模型选择工具还是文本，以及可能的其他候选~~ ✅ `tool_call_decision` 事件 (出站过滤结果);
- ~~Expressor 改写前后对比: 原始文本 ↔ 改写文本~~ ✅ `expressor_rewrite` 事件 (左右分栏对比);
- ~~工具事务尾部: 提取的尾部片段、校验结果、是否被丢弃及原因~~ ✅ `tool_transaction` 事件;
- ~~触发原因: 本次推理的触发原因类型~~ ✅ `trigger_reason` 事件;
...
采用空间级串行策略: 同一 `space_id` 的请求逐条处理, 不同空间并行。
锁键优先级: `space_id > source_user > api_key_id > "global"`。
非流式请求在 `create_chat_completion` 的 try/finally 中释放; 流式请求在
`locked_stream` 包装器的 finally 中释放。
