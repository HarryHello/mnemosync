...
v0.2.x （当前，单人格单用户）
    │
    ├── Phase 1: Agent 简化（v0.2.x 内完成）
    │   移除 LLM 衰减评估 → 确定性公式
    │   提示词清洗 ReAct → 单次重写
    │   其他 Agent 小幅优化
    │
    ├── Phase 2: 多用户基础（v0.3.0）
    │   Actor / Space 身份模型
    │   空间事件流和幂等
    │   受众过滤检索
    │
    └── Phase 3: Agent 统一化（v0.3.x）
...
| 版本 | 定位 | 内容 |
|------|------|------|
| **v0.2.x** | 单人格单用户 | Agent 简化、现有功能完善 |
| **v0.3.0** | 单人格多用户 | 多用户身份、空间、受众基础 |
| **v0.3.x** | 单人格多用户 | Agent 统一契约、可观测性 |

---

## 3. Phase 1: Agent 简化（v0.2.x 内完成）
...
## 5. Phase 2: 多用户基础（v0.3.0）

**目标**：为单人格多用户群聊打下身份和记忆作用域基础。

### 5.1 Actor / Space 身份模型
...
| Actor / Space 身份 | 2 | 多用户基础 | 高 | P3 |
| 受众过滤检索 | 2 | 隐私合规 | 高 | P3 |
| AgentSpec / AgentRun | 3 | 可观测性提升 | 中 | P2 |
| 超时/取消 | 3 | 稳定性提升 | 中 | P2 |
| 版本化结果 | 3 | 一致性提升 | 高 | P3 |
