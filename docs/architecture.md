...
技术上，它是一个反向代理服务器，但其核心价值不在于转发，而在于**上下文编排**与**社交记忆管理**。它拦截标准的 OpenAI 兼容请求，在本地完成人格记忆加载、关系状态解析、记忆访问策略过滤、上下文合并后，再将"纯净"的请求转发给上游模型。
...
1.  **API Gateway**: 入口网关，负责 API Key 鉴权、请求格式校验、Persona ID 路由、`user_identifier` 解析。
...
sequenceDiagram
    ...
    Gateway->>Gateway: 验证 API Key & 解析 Persona ID
...
适用于多用户 SaaS 场景，需引入 Redis 共享会话状态。
