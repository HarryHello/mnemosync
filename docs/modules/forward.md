...
- **仍有限制**: 工具调用和结果尚未作为独立事件持久化；幂等缓存仍只保存文本, 不能重放纯 `tool_calls` 响应；同一逻辑交互的多次 HTTP 请求尚无统一 `interaction_id`；上游候选工具能力声明尚未实现。后续设计见 [群聊与工具演进](../design/group-chat-and-tool-evolution.md)。
