    ...
    async def get(
        ...
        async with self._conn() as db:
            async with db.execute(
                "SELECT integration_id, external_event_id, event_id, response_text, created_at "
    ...
    async def record(
        ...
        async with self._conn() as db:
            await db.execute(
                ...
                "(integration_id, external_event_id, event_id, response_text, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (integration_id, external_event_id, event_id, response_text, now.isoformat()),
            )
            await db.commit()
